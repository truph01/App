...
describe('IOURequestStepTimeRate', () => {
    ...
    describe('rate input and saving', () => {
        ...
        it('should handle decimal rate values', async () => {
            const hours = 5;
            const rate = 3333; // $33.33 in cents
            ...
            expect(setMoneyRequestTimeRateSpy).toHaveBeenCalledWith(TRANSACTION_ID, rate, true);
            // computeTimeAmount(3333, 5) = 16665
            expect(setMoneyRequestAmountSpy).toHaveBeenCalledWith(TRANSACTION_ID, 16665, CONST.CURRENCY.USD);
        });

        it('should not save if count is undefined', async () => {
            await act(async () => {
                await Onyx.merge(`${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${TRANSACTION_ID}`, {
                    transactionID: TRANSACTION_ID,
                    reportID: REPORT_ID,
        ...
        it('should not save if currency is undefined', async () => {
            await act(async () => {
                await Onyx.merge(`${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${TRANSACTION_ID}`, {
                    transactionID: TRANSACTION_ID,
                    reportID: REPORT_ID,
    ...
    describe('merchant string formatting', () => {
        it('should format merchant with hours and rate', async () => {
            const hours = 8;
            const rate = 5500; // $55.00 in cents
            ...
            // Merchant string should contain the hours count and rate info
            expect(merchantCall?.at(1)).toContain('8');
        });

        it('should format merchant with decimal hours', async () => {
            const hours = 5.5;
            const rate = 4200; // $42.00 in cents
            ...
            expect(setMoneyRequestMerchantSpy).toHaveBeenCalled();
            const merchantCall = setMoneyRequestMerchantSpy.mock.calls.at(0) as unknown[] | undefined;
            expect(merchantCall?.at(1)).toContain('5.5');
        });
    });
});
