...
import {replaceReceipt} from '@userActions/IOU';
...
jest.mock('@userActions/IOU', () => {
    const actual = jest.requireActual<typeof IOU>('@userActions/IOU');
    return {
        ...actual,
...
describe('MoneyRequestReceiptView', () => {
    const allReports: OnyxCollection<Report> = {
        [`${ONYXKEYS.COLLECTION.REPORT}${TEST_REPORT_ID}`]: testReport,
        [`${ONYXKEYS.COLLECTION.REPORT}${TEST_PARENT_REPORT_ID}`]: testParentReport,
    };
    ...
    beforeEach(async () => {
        jest.clearAllMocks();
        global.URL.createObjectURL = jest.fn(() => 'blob:mock-receipt-url');
        await act(async () => {
    ...
    describe('wide RHP setReceiptFile flow', () => {
        it('calls setReceiptFile with the picked file when onPicked is invoked (replaceReceipt called)', async () => {
            ...
            const pickedFile = new File(['receipt content'], 'receipt.pdf', {type: 'application/pdf'}) as FileObject;
            onPicked?.([pickedFile]);
            await waitForBatchedUpdatesWithAct();

            expect(replaceReceipt).toHaveBeenCalledTimes(1);
            expect(replaceReceipt).toHaveBeenCalledWith(
