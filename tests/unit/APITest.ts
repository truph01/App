...
describe('API.write() persistence guarantees', () => {
    it.failing('Issue 1: should persist the request before applying optimistic data', () =>
        Onyx.multiSet({
            [ONYXKEYS.CREDENTIALS]: {autoGeneratedLogin: 'test', autoGeneratedPassword: 'passwd'},
            [ONYXKEYS.SESSION]: {authToken: 'testToken'},
            [ONYXKEYS.NETWORK]: {isOffline: true},
        }).then(() => {
            // Set up spies AFTER setup to only capture calls from API.write()
            const setMock = jest.spyOn(Onyx, 'set');
            const updateMock = jest.spyOn(Onyx, 'update');

            try {
                // @ts-expect-error -- passing mock command with onyxData
                ...
                // Find the Onyx.set call that persisted the request to disk
                const persistCallIdx = setMock.mock.calls.findIndex(([key]) => key === ONYXKEYS.PERSISTED_REQUESTS);
                expect(persistCallIdx).toBeGreaterThanOrEqual(0);
                expect(updateMock).toHaveBeenCalled();

                // The request must be durably persisted via Onyx.set(PERSISTED_REQUESTS) BEFORE
                // optimistic data is applied to the UI via Onyx.update. This ensures that if the
                // app crashes right after optimistic updates are visible, the write request survives.
                const persistOrder = setMock.mock.invocationCallOrder[persistCallIdx];
                const updateOrder = updateMock.mock.invocationCallOrder[0];
                expect(persistOrder).toBeLessThan(updateOrder);
            } finally {
                setMock.mockRestore();
