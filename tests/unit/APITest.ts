...
describe('API.write() persistence guarantees', () => {
    const TEST_OPTIMISTIC_KEY = 'test_optimisticDataMarker';
    ...
    it.failing('Issue 1: should persist the request before applying optimistic data', () =>
        Onyx.multiSet({
            ...
            const updateMock = jest.spyOn(Onyx, 'update').mockImplementation((data) => {
                const hasMarker = data.some((entry) => entry.key === TEST_OPTIMISTIC_KEY);
                if (hasMarker) {
            ...
            try {
                API.write('MockCommand' as WriteCommand, {param1: 'value1'} as ApiRequestCommandParameters[WriteCommand], {
                    optimisticData: [
                        {
                            onyxMethod: Onyx.METHOD.SET,
                            key: TEST_OPTIMISTIC_KEY,
                            value: true,
                        },
                    ],
                });

                // The request must already be in the persisted queue BEFORE optimistic data
                // is applied via Onyx.update. This ensures that if the app crashes right after
                // optimistic updates are visible, the write request survives on disk.
                expect(requestPersistedBeforeOptimistic).toBe(true);
            } finally {
