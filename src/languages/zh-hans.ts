...
（例如：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    twoFactorAuth: {
        ...
        explainProcessToRemove: '若要停用双重身份验证（2FA），请输入来自您身份验证应用中的有效验证码。',
        explainProcessToRemoveWithRecovery: '若要禁用双重身份验证（2FA），请输入有效的恢复代码。',
        disabled: '已关闭双重身份验证',
...
_如需更详细的说明，请[访问我们的帮助网站](${CONST.NETSUITE_IMPORT.HELP_LINKS.CUSTOM_SEGMENTS})_。`,
                        ...
                        errors: {uniqueFieldError: ({fieldName}: RequiredFieldParams) => `已存在使用此 ${fieldName?.toLowerCase()} 的自定义分段/记录`},
                    },
                    customLists: {
                        title: '自定义列表',
                        addText: '添加自定义列表',
...
你可以要求报销包含收据和说明等详细信息，设置限额和默认值，并将审批和付款流程自动化——全部在一个地方完成。`,
    ...
    cardTransactions: {notActivated: '未启用', outOfPocket: '自付支出', companySpend: '公司支出'},
    distance: {
        addStop: '添加站点',
        address: '地址',
        waypointDescription: {
...
这里有一张*测试收据*，来展示它是如何运作的：`,
    ...
    domain: {
        ...
        members: {
            ...
            closeAccountInfo:
                '我们建议安全关闭账户，以免在以下情况下无法关闭：<ul><li>有待处理的审批</li><li>有正在进行的报销</li><li>没有其他登录方式</li></ul>否则，你可以忽略以上安全防范措施并强制关闭所选账户。',
            error: {
