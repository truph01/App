...
function getCategorySections(data: OnyxTypes.SearchResults['data'], queryJSON: SearchQueryJSON | undefined): [TransactionCategoryGroupListItemType[], number] {
    ...
    for (const key in data) {
        if (isGroupEntry(key)) {
            ...
            const transactionsQueryJSON =
                queryJSON && categoryGroup.category !== undefined ? buildGroupDrilldownQuery(queryJSON, CONST.SEARCH.SYNTAX_FILTER_KEYS.CATEGORY, categoryValue) : undefined;
...
function getMerchantSections(data: OnyxTypes.SearchResults['data'], queryJSON: SearchQueryJSON | undefined, translate: LocalizedTranslate): [TransactionMerchantGroupListItemType[], number] {
    ...
    for (const key in data) {
        if (isGroupEntry(key)) {
            ...
            const transactionsQueryJSON =
                queryJSON && merchantGroup.merchant !== undefined ? buildGroupDrilldownQuery(queryJSON, CONST.SEARCH.SYNTAX_FILTER_KEYS.MERCHANT, merchantValue) : undefined;
...
function getTagSections(data: OnyxTypes.SearchResults['data'], queryJSON: SearchQueryJSON | undefined, translate: LocalizedTranslate): [TransactionTagGroupListItemType[], number] {
    ...
    for (const key in data) {
        if (isGroupEntry(key)) {
            ...
            const transactionsQueryJSON = queryJSON && tagGroup.tag !== undefined ? buildGroupDrilldownQuery(queryJSON, CONST.SEARCH.SYNTAX_FILTER_KEYS.TAG, tagValue) : undefined;
...
function getSortedSections(
    ...
    if (groupBy) {
        const sortFn = groupBySortFunction[groupBy];
        if (sortFn) {
            return sortFn(data as TransactionGroupListItemType[], localeCompare, sortBy, sortOrder);
        }
    }
