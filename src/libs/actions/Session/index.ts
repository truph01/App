 ...
 * For full sign-out including Onyx cleanup and redirect, use signOutAndRedirectToSignIn().
 */
function signOut(params: {autoGeneratedLogin?: string; signedInWithSAML?: boolean; authToken?: string} = {}): Promise<void | Response> {
...
function callSAMLSignOut(params: LogOutParams, authToken: string): Promise<void | Response> {
...
function signOutAndRedirectToSignIn(shouldResetToHome?: boolean, shouldStashSession?: boolean, shouldSignOutFromOldDot = true, shouldForceUseStashedSession?: boolean) {
    ...
    // session.
    const signOutPromise: Promise<void | Response> =
