...
DOMPurify.addHook("afterSanitizeAttributes", (node) => {
  if (node.tagName === "A" && node.hasAttribute("href")) {
    ...
    try {
      ...
      const isExpensifyLink =
        hostname === "expensify.com" || hostname.endsWith(".expensify.com");
      if (
        !isExpensifyLink &&
  ...
  fetch(SEARCH_API_URL, { method: "POST", body: formData })
    ...
    .then((data) => {
      const results = (data.searchResults || []).filter(
        (result) => !result.url.includes("/Unlisted/"),
      );
      resultsContainer.innerHTML = "";
      if (results.length === 0) {
        resultsContainer.appendChild(
          cloneTemplate("search-no-results-template"),
        );
        return;
      }
      results.forEach((result) => {
        ...
        link.querySelector(".search-result-title").textContent =
          getTitleFromURL(result.url);
    ...
    .then((data) => {
      ...
      if (renderedContent.scrollHeight > renderedContent.clientHeight) {
        ...
        showMoreButton.addEventListener("click", () => {
          ...
          showMoreButton.firstChild.textContent =
            renderedContent.classList.contains("expanded")
              ? "Show less "
              : "Show more ";
        });
      }
...
function initSearchPage() {
  ...
  document
    .getElementById("search-page-clear")
